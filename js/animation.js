...
function animLoadDefault() {
  ...
  makeDefaultImage(animN).then(d => {
    animOrigData    = d;
    animCurrentData = new ImageData(new Uint8ClampedArray(d.data), animN, animN);
    animIterations  = 0;
...
function animOnUpload(input) {
  ...
  if (!file) return;
  animN = Math.min(500, Math.max(2, parseInt(document.getElementById('anim-size').value) || 16));
...
function animDetectPeriod() {
  ...
  let cur    = new ImageData(new Uint8ClampedArray(animOrigData.data), animN, animN);
...
function animStep(dir) {
  ...
  if (dir === 1) {
    if (animHistory.length > animIterations + 1) {
      ...
      animHistory.push(new ImageData(new Uint8ClampedArray(next.data), animN, animN));
...
function animSaveFrame() {
  ...
  const filename = animFilename ? animFilename.replace(/\.[^/.]+$/, "") : "arnold-cat-map";
